...
#Due Date 7/04/2022
#This program performs similarly to the task2.py program, however this time a checksum check is implemented. This is done to ensure that the packet
#which is recieved is complete and error-free. This program also sends a packet to the server, which in turn will echo back the current date and time until interrupted.
...
#Defines the asynchronous function send_Packet.
async def send_Packet(websocket, source_port: int, dest_port: int, payload:bytearray):
    #Creates the values to use for the construction of the packet that will be sent back to the server. This code is similar to that in the compute_checksum function.
    sizeSend = (8 + len(payload)).to_bytes(2, byteorder = 'little')
    source_portSend = source_port.to_bytes(2, byteorder = 'little')
    dest_portSend = dest_port.to_bytes(2, byteorder = 'little')
    checksumSend = compute_checksum(source_port, dest_port, payload).to_bytes(2, byteorder = 'little')

    #Combines all of the previously created values within the function to create a full packet.
    prepPacket = source_portSend + dest_portSend + sizeSend + checksumSend + payload

    #Takes the completed packet and encodes it, using Base64.
    packetSend = base64.b64encode(prepPacket)
    #Sends the packet to the server.
    await websocket.send(packetSend)
...
async def main():
    ...
    async with websockets.connect(uri) as websocket:
        ...
        #"While True" is an infinite loop here. The program will continue until interrupted.
        while True:
            #Perform the send_Packet function, with the values 0, 542 and b'1111'.
            await send_Packet(websocket, 0, 542, b'1111')
            #Waits for the decode_Packet function to complete.
            await decode_Packet(websocket)
            #Forces the program to wait for 1 second before running the loop again.
            time.sleep(1)
